...
export const Route = createFileRoute("/course/$courseId")({
	...
	loader: async ({ params }) => {
		...
		if (
			...
			hardcodedCourses.find((c: Course) => c.id === courseId)
		) {
		...
		<main className="page-wrap px-4 pb-8 pt-14">
			...
			<div className="rise-in mb-6 flex items-center gap-3 sm:gap-4">
				...
				<div className="min-w-0">
					...
					<h1 className="truncate text-lg font-bold text-[var(--sea-ink)] sm:text-2xl">
						{course.name}
					</h1>
				</div>
			</div>
			...
			{/* Tabs */}
			<div
				className="rise-in mb-6 flex gap-1 rounded-xl border border-[var(--line)] bg-[var(--surface-strong)] p-1"
			...
			<div className="rise-in" style={{ animationDelay: "160ms" }}>
				{activeTab === "material" ? (
					...
					<ExerciseSection
						content={content}
						isGenerated={generatedContent !== null}
					/>
				)}
			</div>
		</main>
