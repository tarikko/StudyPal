...
// Read env vars from import.meta.env (Vite — works on both server and browser).
// On the server all vars are available; in the browser only VITE_-prefixed ones
// are included in the bundle. Mirror each var with a VITE_ counterpart in your
// .env file to expose them to the browser (e.g. VITE_MISTRAL_API_KEY=...).
function getEnvVar(name: string): string | undefined {
  const env = import.meta.env as Record<string, string | undefined>;
  return env[name] ?? env[`VITE_${name}`];
}

const OPEN_SOURCE_OCR_BASE = getEnvVar("OPEN_SOURCE_OCR_BASE_URL")?.replace(
  /\/$/,
  "",
);
const OPEN_SOURCE_OCR_MODEL = getEnvVar("OPEN_SOURCE_OCR_MODEL");
const OPEN_SOURCE_OCR_API_KEY = getEnvVar("OPEN_SOURCE_OCR_API_KEY");

function getKey(): string {
  const key = getEnvVar("MISTRAL_API_KEY");
...
async function ocrImageWithOpenSource(
  base64: string,
  mimeType: string,
): Promise<string> {
